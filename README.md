...
> ⚠️ **前置条件**：流量进度条的数据来自面板的**流量统计告警规则**（类型 `transfer_all_cycle`，配置了流量上限和计费周期）。没有该规则的服务器，前台不会显示进度条。每台要显示流量的服务器都需要一条启用的流量统计规则。

模板见 [流量统计告警规则模板](docs/traffic-alert-rule-template.md)（含字段说明和面板真实示例）。
